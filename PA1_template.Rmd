...
```{r withNoNa}
mdf <- df %>% group_by(interval) %>% mutate(mstep = mean(steps, na.rm = TRUE))
nona_df <- mdf
head(nona_df,10)
nona_df$steps <- ifelse(is.na(nona_df), nona_df$sm, nona_df$steps)
# nona_df$date <- ymd(nona_df$date)
# names(nona_df) <- c("Date","Steps")
# g <- ggplot(nona_df, aes(x = Date, y = Steps)) +
#     geom_histogram(stat = "identity")
# print(g)
na_mn <- mean(nona_df$Steps, na.rm = TRUE)
na_md <- median(nona_df$Steps, na.rm = TRUE)
```
